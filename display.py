...
class Display:
    ...
    def update_leds(self, array=np.ones((7, 17)), incremental=False, dt_incremental=0.05, dt_func=lambda t: 0.9 ** t,color=(255,0,255)):
        with self.lock:
            self.uh.clear()
            for w in range(self.w):
                r, g, b = color
            ...
            self.uh.show()
    def blink_count(self, count=20, dt_pause=0.2, dt_on=0.4, dt_func=lambda t: 0.93 ** t, color_cycles=10, color=(255,0,255)):
        color_cycler = get_color_cycler(color_cycles)()
    ...
    def blink_time(self, _time=7, dt_pause=0.2, dt_on=0.4, dt_func=lambda t: 0.7 ** t, color_cycles=10, color=(255,0,255)):
        time_st = time.time()
        color_cycler = get_color_cycler(color_cycles)()
